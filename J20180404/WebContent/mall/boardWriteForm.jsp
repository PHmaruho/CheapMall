...
<script>
	...
	function popUp() {
		window
				.open('boardWriteFormPopup.mall', '상품찾기',
...
<body>
			...
			<label>구분</label> <select id="select" onchange="list()">
			...
			<label>일반</label><input type="radio" name="radio" onclick="radio(0)"
				checked="checked"> <label>상품</label><input type="radio"
				name="radio" onclick="radio(1)">
			<div id="suggestDiv" class="none">
				<input type="text" disabled="disabled" id="suggestText"
					name="suggestText"> <input type="button" value="상품찾기"
					onclick="popUp()">
			</div>

			<label>대상자 ID</label> <input type="text" id="reportText"
				oninput="javascript : $('#checkId').val('0');"
				placeholder="ID를 입력하세요" autofocus> <input type="button"
				onclick="winop()" style="background-color: #EDCE7A" value="아이디 확인"
				id="idBtn"> <input type="hidden" value="0" id="checkId">
			<div id="idChk"></div>

		<br>

			<form action="boardWritePro.mall" method="post"
				onsubmit="return check()">
				<label>제목</label><input type="text" required="required"
					name="subject"><br>
				<textarea rows="20" cols="50" required="required" id="content"
					name="content"></textarea>
				<br> <input type="hidden" id="hidden" name="board_cd">
